...
function BaseTable(props: Props) {
  ...
  const [columns, setColumns] = useState(
    filterTableColumns(props.columns as TableColumn[])
  );
    ...
    const result = newColumns.map((col, index) => ({
      ...
      render: (value: unknown, record: object, index: number) => {
        ...
        if (Array.isArray(showValue)) {
          ...
          if (isStringArr)
            showValue = showValue?.join(", ") || config.EMPTY_PLACEHOLDER;
        }

        if (
          !["object", "function"].includes(typeof renderContent) ||
          isStringArr
        ) {
          const textContent = String(showValue ?? config.EMPTY_PLACEHOLDER);
        ...
        >
          ...
          >
            ...
            <Button
              ...
              icon={<RedoOutlined disabled={!!isLoading} />}
              onClick={getPage}
            >
              {t("刷新")}
            </Button>
