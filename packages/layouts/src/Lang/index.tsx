...
function I18n() {
  ...
  const [lang, setLang] = useState(
    localStorage.getItem(config.LANG_KEY) || config.DEFAULT_LANG
  );

  useEffect(() => {
    // 本地缓存的语言
    const lcalLang = localStorage.getItem(config.LANG_KEY);

    // i18正在使用的语言
    const il8nLang = i18n.language;

    if (!lcalLang) {
      localStorage.setItem(config.LANG_KEY, config.DEFAULT_LANG);
      i18n.changeLanguage(config.DEFAULT_LANG);
  ...
  const onClick: MenuProps["onClick"] = (e) => {
    ...
    localStorage.setItem(config.LANG_KEY, e.key);
    setLang(e.key);
    handleSetTitle();
  };
