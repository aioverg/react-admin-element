...
import { setTitle, encryption, decryption, config } from "@packages/utils";
...
function Login() {
  ...
  const themeCache = (localStorage.getItem(config.THEME_KEY) ||
    "light") as ThemeType;
  ...
  const confirm: FormProps["onFinish"] = async (values: LoginData) => {
    try {
      ...
      handleRemember(
        values.username,
        encryption({ value: values.password, expire: 0 })
      );
      ...
      >
        ...
        <div
          style={{
            ...
            top: "calc(50% - 185px)",
            boxShadow:
              "2px 5px 20px var(--un-shadow-color, rgba(0, 0, 0, 0.1))",
