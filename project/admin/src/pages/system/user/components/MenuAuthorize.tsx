...
function MenuAuthorize(props: Props) {
  ...
  useEffect(() => {
    setCheckedKeys(
      defaultCheckedKeys?.length
        ? defaultCheckedKeys.map((item) => item?.toString())
        : []
    );
  }, [defaultCheckedKeys]);
    ...
    return (
      ...
      >
        {list.map((item) => {
          ...
          const childrenKeys = getChildKeys(item);
          const isEmptyChildrenKeys = !childrenKeys?.some((child) =>
          ...
          const isIndeterminate =
            item?.children?.length && !isEmptyChildrenKeys
              ? !isAllCheckedChildrenKeys
              : undefined;
          ...
          return (
            <div
              key={item.key}
              style={{ display: item.type === 3 ? "inline-block" : "block" }}
            >
              <div
                style={{
              ...
              {hasChildren &&
                isExpanded &&
                renderTreeNode(item.children!, step + 1)}
            </div>
          );
        })}
      </div>
  ...
  const isAllChecked =
    ...
    allKeys.every((item) => checkedKeys.includes(item));
    ...
    <Spin spinning={isLoading}>
      <div>
        ...
        >
          ...
          >
            {expandedKeys.length === getAllExpandableKeys(list).length
              ? "收起所有"
              : "展开所有"}
          </div>
        </div>
